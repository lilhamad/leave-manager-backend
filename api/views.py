...
def my_leaves(request, id):
    ...
    permission_classes = (IsAuthenticated,)
    authentication_classes = (TokenAuthentication,) 
    leaves = Leave.objects.filter(employee_id=id)
...
def create_leave(request):
    ...
    # authentication_classes = (TokenAuthentication,) 
    # print(request.data)
    leaves = Leave.objects.create(start_date= request.data.get("start_date"), end_date= request.data.get("end_date"), application_date= request.data.get("application_date"), employee = Employee.objects.get(id = request.data.get("employee")))
    print('GRRRRRRREEEEEEEEEEEEEAAAAAAAAAAAAAAAT')
    leaves = Leave.objects.filter(employee_id=request.data.get("employee"))
    serializer = LeaveSerializer(leaves, many=True, read_only=True, context=serializer_context)
    return JsonResponse(serializer.data, safe=False)
